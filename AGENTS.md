...
`docs/api/openapi.yaml` 是接口主契约。后端响应、前端 API 类型、mock 数据和契约测试都必须与它一致。
...
双方都可以读取全部文档。接口需要变化时，先更新 `docs/api/openapi.yaml`，再改实现和测试。
...
所有写操作最终都必须包含 RBAC、审计日志、事务边界和测试。
