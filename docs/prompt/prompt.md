...
> 版本：`wiki_prompt_v0.4`
> 状态：在 `wiki_prompt_v0.3` Micro Eval 后，将针对失败项的规则抽象为通用身份、证据、事实、关系和噪音判定框架，提升跨数据集泛化能力。
...
```text
你是 Wiki 引用标注器。只输出合法 JSON，不要输出解释；chunk_id 必须来自输入 chunks。引用选择要服务于证据覆盖，不针对单一字段补丁。
```
...
6. 已基于 `wiki_prompt_v0.3` 重跑 Micro Eval，并将失败归因继续抽象为 `wiki_prompt_v0.4` 的身份、证据、事实、关系和噪音判定框架。
7. 后续基于 `wiki_prompt_v0.4` 重跑 Micro Eval 与 Scenario Eval，重点观察跨数据集稳定性，而不是单个样例命中。
