...
| 4.2.2 | 知识库列表（分区、搜索、筛选） | 部分：骨架 M1，文档数/状态 M2 补全 | M1（已完成）、M2 |
| 4.2.3 | 文档标签（多标签、筛选、增删改） | v1 | M2 |
| 4.3.1 | 支持格式（.md / .txt） | v1 | M2 |
| 4.3.2 | 文件上传（SHA-256 去重、批量、异步状态、重试、选标签） | v1 | M2 |
| 4.3.3 | 文档列表与详情（预览、chunk 列表、元信息） | v1 | M2 |
| 4.3.4 | 文档删除（级联清理、审计） | 部分：来源页"来源已删除"标记后置 | M2 |
| 4.4.1 | Header 路径感知切块（Markdown） | v1 | M2 |
| 4.4.2 | 超长章节二次切分（overlap） | v1 | M2 |
| 4.4.3 | 纯文本切分（TXT） | v1 | M2 |
| 4.4.4 | Chunk 数据结构 | v1 | M2 |
| 4.4.5 | 向量化上下文拼接（header_path 拼入 embedding 与 BM25） | v1 | M2 |
| 4.4.6 | 分块预览 | v1 | M2 |
...
| 5.2 | 安全（bcrypt、AES、隔离、上传限制、审计） | v1：随里程碑实现，M5 集中验证 | M1-M4、M5 |
| 5.3 | 部署（Docker Compose、持久化卷） | v1：M0 基线，M5 空库部署演练 | M0（已完成）、M5 |
| 5.4 | 可用性（失败隔离、重试、部分成功） | v1 | M2、M3 |
| 5.5 | 国际化（界面中文） | v1 | 全程 |
